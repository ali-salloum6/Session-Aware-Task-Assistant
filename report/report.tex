...
\paragraph{Course deliverables.}
The submission consists of three artefacts:
\begin{enumerate}[leftmargin=*]
  \item \textbf{Report} --- this LaTeX document (PDF).
  \item \textbf{Repository} --- source code at
    \url{https://github.com/ali-salloum6/Session-Aware-Task-Assistant}.
  \item \textbf{Video} --- a live demo of the project running on a real OpenRouter run
    (preference write/recall, tools, local traces, Langfuse, and evaluation tables).
\end{enumerate}
...
P9 report (draft) & Ali Salloum & --- \\
P9 report (methods/eval) & Ajibade Ayomide & --- \\
P9 video (stable live run) & Ali Salloum & --- \\
\bottomrule
\end{tabularx}
\end{table}
...
The three course deliverables are:
\begin{enumerate}[leftmargin=*]
  \item \textbf{Report:} this PDF (\texttt{report/report.pdf}; source \texttt{report/report.tex}).
  \item \textbf{Repository:}
    \url{https://github.com/ali-salloum6/Session-Aware-Task-Assistant}
    (includes \texttt{src/assistant/}, \texttt{scripts/}, \texttt{eval/},
    \texttt{docs/architecture.md}, \texttt{docs/memory-policy.md}).
  \item \textbf{Video:} live demo of a real run (bundled with the submission package).
\end{enumerate}
